...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        trusted_utils_try_match_bool(argv[i], "-palrup-binary=", &use_palrup_binary);
    }
    ...
    pc_init(formula_path, proofs_path_in, proofs_path_out, solver_id, num_solvers, redistribution_strategy, read_buffer_size, use_palrup_binary);
    int res = pc_run();
    pc_end();
    fflush(stdout);
    return res;
